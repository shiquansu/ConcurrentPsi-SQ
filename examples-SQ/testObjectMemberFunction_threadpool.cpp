...
#include "../src-SQ_c++11/ObjMemFuncThreadPool.h"
...
int main(int argc, char* argv[]){
	...
	ConcurrentPsi::ObjMemFuncThreadPool testThreadPool;
	...
	// submit VBlock method printVIndex as function to the ObjMemFuncThreadPool, ObjMemFuncThreadPool use a thread safe workQueue to hold these functions and then the thread work from the queue.
	using VBPtrType = void(VBlock::*)();
	VBPtrType printVIndexPtr = &VBlock::printVIndex;
	printVIndexPtr = printVIndexPtr;
	for (SizeType i=0; i<job_num; i++){
		testThreadPool.submit( V[i], printVIndexPtr);
	}	
